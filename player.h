...
#define PlayerAttackType (13) //現状6 弱、下、横、上、つかみ、shot
#define AttackStatus (8)
...
private:
	...
	float m_AttackStatus[PlayerAttackType][AttackStatus];//Pos:x y Size:x y ZFrame:? Angle:? KBG:? BKB? Damage:? Dis:x y
	int   StoreNum = 0;

	//AttackStatus関連
	float m_AttackSize[PlayerAttackType][2];//x,y
	int	  m_AttackZframe[PlayerAttackType][1];//zframe
	float m_AttackAngle[PlayerAttackType][1];//angle
	float m_AttackSpeed[PlayerAttackType][1];//speed
	float m_AttackDamage[PlayerAttackType][1];//damage
	float m_AttackDis[PlayerAttackType][2];//disx,disy
	...
	bool m_IsChance = false;//現状trueはガード硬直中

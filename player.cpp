	...
	if (m_HideTime == 0)
	{
		...
		if (m_AnimationName == "Punch")
		{
			...
			if (m_AnimationFrame == 13 && m_isAttack[0] == true)
			{
				...
				AddCollid(m_Position.x, m_Position.y, m_AttackSize[0][0], m_AttackSize[0][1], m_AttackZframe[0][0], m_FaceIsRight, m_AttackAngle[0][0], m_AttackSpeed[0][0], m_AttackDamage[0][0], m_AttackDis[0][0], m_AttackDis[0][1]);
			}
		...
		if (m_AnimationName == "DownTilt")
		{
			...
			{
				AddCollid(m_Position.x, m_Position.y, m_AttackSize[1][0], m_AttackSize[1][1], m_AttackZframe[1][0], m_FaceIsRight, m_AttackAngle[1][0], m_AttackSpeed[1][0], m_AttackDamage[1][0], m_AttackDis[1][0], m_AttackDis[1][1]);
			}
		...
		if (m_AnimationName == "ForwardTilt")
		{
			...
			{
				AddCollid(m_Position.x, m_Position.y, m_AttackSize[2][0], m_AttackSize[2][1], m_AttackZframe[2][0], m_FaceIsRight, m_AttackAngle[2][0], m_AttackSpeed[2][0], m_AttackDamage[2][0], m_AttackDis[2][0], m_AttackDis[2][1]);
			}
		...
		if (m_AnimationName == "Upper")
		{
			...
			{
				AddCollid(m_Position.x, m_Position.y, m_AttackSize[3][0], m_AttackSize[3][1], m_AttackZframe[3][0], m_FaceIsRight, m_AttackAngle[3][0], m_AttackSpeed[3][0], m_AttackDamage[3][0], m_AttackDis[3][0], m_AttackDis[3][1]);
			}
		...
		if (m_AnimationName == "Grab")
		{
			...
			{
				AddCollid(m_Position.x, m_Position.y, m_AttackSize[4][0], m_AttackSize[4][1], m_AttackZframe[4][0], m_FaceIsRight, m_AttackAngle[4][0], m_AttackSpeed[4][0], m_AttackDamage[4][0], m_AttackDis[4][0], m_AttackDis[4][1]);
			}
...
void Player::InitAttackStatus()
{
	for (int i = 0; i < PlayerAttackType; i++)
	{
		for (int j = 0; j < 11; j++)
		{
			m_AttackStatus[i][j] = 0.0f;
		}
	}
	
}

void Player::LoadAttackStatus()
{
	std::string g_SaveFileLocation = "./asset/data/AttackStatus.txt";

	std::ifstream LoadFile;
	LoadFile.open(g_SaveFileLocation);
	if (LoadFile.fail()) {
		std::cerr << "Failed to open file!" << std::endl;
		return;
	}

	for (int i = 0; i < 5; i++)//弱、下、横、上、つかみ、
	{
		string s;
		LoadFile >> s;//技の名前
		...
		for (int j = 0; j < 6; j++) {
			while (true) {
				...
				char now = LoadFile.get();

				if (LoadFile.fail()) {
					std::cerr << "Failed to read character from file!" << std::endl;
					return;
				}
			...
			switch (j) {
			case 0:
				LoadFile >> m_AttackSize[i][0];
				LoadFile >> m_AttackSize[i][1];
				break;
			case 1:
				LoadFile >> m_AttackZframe[i][0];
				break;
			case 2:
				LoadFile >> m_AttackAngle[i][0];
				break;
			case 3:
				LoadFile >> m_AttackSpeed[i][0];
				break;
			case 4:
				LoadFile >> m_AttackDamage[i][0];
				break;
			case 5:
				LoadFile >> m_AttackDis[i][0];
				LoadFile >> m_AttackDis[i][1];
				break;
			}

			if (LoadFile.fail()) {
				std::cerr << "Failed to read data from file at case " << j << std::endl;
				return;
			}
		}
	}
...
{
	ImGui::SetNextWindowSize(ImVec2(700.0, 200.0));

	if (ImGui::Begin("Attack Settings", NULL, ImGuiWindowFlags_AlwaysAutoResize))
	{
		// タブバーの作成
		if (ImGui::BeginTabBar("Attack Tabs"))
		{
			// タブ1
			if (ImGui::BeginTabItem("Jab"))
			{
				ImGui::Text("This is Tab 1.");
				ImGui::SliderFloat("SizeX", &m_AttackSize[0][0], 0.0f, 10.0f);
				ImGui::SliderFloat("SizeY", &m_AttackSize[0][1], 0.0f, 10.0f);
				ImGui::SliderInt("ZFrame", &m_AttackZframe[0][0], 0, 10);
				ImGui::SliderFloat("Angle", &m_AttackAngle[0][0], 0.0f, 180.0f);
				ImGui::SliderFloat("Speed", &m_AttackSpeed[0][0], 0.0f, 5.0f);
				ImGui::SliderFloat("Damage", &m_AttackDamage[0][0], 0.0f, 40.0f);
				ImGui::SliderFloat("DisX", &m_AttackDis[0][0], 0.0f, 10.0f);
				ImGui::SliderFloat("DisY", &m_AttackDis[0][1], 0.0f, 10.0f);

				ImGui::EndTabItem();
			}

			// タブ2
			if (ImGui::BeginTabItem("DownTilt"))
			{
				ImGui::Text("This is Tab 2.");
				ImGui::SliderFloat("SizeX", &m_AttackSize[1][0], 0.0f, 10.0f);
				ImGui::SliderFloat("SizeY", &m_AttackSize[1][1], 0.0f, 10.0f);
				ImGui::SliderInt("ZFrame", &m_AttackZframe[1][0], 0, 10);
				ImGui::SliderFloat("Angle", &m_AttackAngle[1][0], 0.0f, 180.0f);
				ImGui::SliderFloat("Speed", &m_AttackSpeed[1][0], 0.0f, 5.0f);
				ImGui::SliderFloat("Damage", &m_AttackDamage[1][0], 0.0f, 40.0f);
				ImGui::SliderFloat("DisX", &m_AttackDis[1][0], 0.0f, 10.0f);
				ImGui::SliderFloat("DisY", &m_AttackDis[1][1], 0.0f, 10.0f);

				ImGui::EndTabItem();
			}

			// タブ3
			if (ImGui::BeginTabItem("ForwardTilt"))
			{
				ImGui::Text("This is Tab 3.");
				ImGui::SliderFloat("SizeX", &m_AttackSize[2][0], 0.0f, 10.0f);
				ImGui::SliderFloat("SizeY", &m_AttackSize[2][1], 0.0f, 10.0f);
				ImGui::SliderInt("ZFrame", &m_AttackZframe[2][0], 0, 10);
				ImGui::SliderFloat("Angle", &m_AttackAngle[2][0], 0.0f, 180.0f);
				ImGui::SliderFloat("Speed", &m_AttackSpeed[2][0], 0.0f, 5.0f);
				ImGui::SliderFloat("Damage", &m_AttackDamage[2][0], 0.0f, 40.0f);
				ImGui::SliderFloat("DisX", &m_AttackDis[2][0], 0.0f, 10.0f);
				ImGui::SliderFloat("DisY", &m_AttackDis[2][1], 0.0f, 10.0f);

				ImGui::EndTabItem();
			}

			// タブ4
			if (ImGui::BeginTabItem("UpTilt"))
			{
				ImGui::Text("This is Tab 4.");
				ImGui::SliderFloat("SizeX", &m_AttackSize[3][0], 0.0f, 10.0f);
				ImGui::SliderFloat("SizeY", &m_AttackSize[3][1], 0.0f, 10.0f);
				ImGui::SliderInt("ZFrame", &m_AttackZframe[3][0], 0, 10);
				ImGui::SliderFloat("Angle", &m_AttackAngle[3][0], 0.0f, 180.0f);
				ImGui::SliderFloat("Speed", &m_AttackSpeed[3][0], 0.0f, 5.0f);
				ImGui::SliderFloat("Damage", &m_AttackDamage[3][0], 0.0f, 40.0f);
				ImGui::SliderFloat("DisX", &m_AttackDis[3][0], 0.0f, 10.0f);
				ImGui::SliderFloat("DisY", &m_AttackDis[3][1], 0.0f, 10.0f);
			...
			if (ImGui::BeginTabItem("Grab"))
			{
				...
				ImGui::SliderFloat("SizeX", &m_AttackSize[4][0], 0.0f, 10.0f);
				ImGui::SliderFloat("SizeY", &m_AttackSize[4][1], 0.0f, 10.0f);
				ImGui::SliderInt("ZFrame", &m_AttackZframe[4][0], 0, 10);
				ImGui::SliderFloat("Angle", &m_AttackAngle[4][0], 0.0f, 180.0f);
				ImGui::SliderFloat("Speed", &m_AttackSpeed[4][0], 0.0f, 5.0f);
				ImGui::SliderFloat("Damage", &m_AttackDamage[4][0], 0.0f, 40.0f);
				ImGui::SliderFloat("DisX", &m_AttackDis[4][0], 0.0f, 10.0f);
				ImGui::SliderFloat("DisY", &m_AttackDis[4][1], 0.0f, 10.0f);

...
ID3D11ShaderResourceView* EnemyName::m_Texture{};//static再宣言

void EnemyName::Load()
{
	//テクスチャ読み込み
	D3DX11CreateShaderResourceViewFromFile(Renderer::GetDevice(), "asset\\texture\\cp.png", NULL, NULL, &m_Texture, NULL);
	assert(m_Texture);
}

void EnemyName::Unload()
{
	m_Texture->Release();
}

void EnemyName::Init(float x, float y)
{
